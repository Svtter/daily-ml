...
CONFIG = {
    ...
    'output_repo': "git@github.com:Svtter/daily-ml.git",
    # Port for `serve`
    'port': 8000,
}
...
def update(c):
    """update remote repo"""
    c.run('git add .')
    c.run('git commit -m "udpate content"')
    c.run('git push')


@task
def post(c):
    name = input('input the post name: ')
    title = f'Title: {name}'
    date = arrow.now().format('YYYY-MM-DD')
    datetime = f"Date: {arrow.now().format('YYYY-MM-DD HH:mm')}"
    category = f"Category: Review"

    filename = f'content/{date}-{name}.md'
